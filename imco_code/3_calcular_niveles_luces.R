...
pibe_16 <- read_csv("../data/bie/processed/pibe.csv") %>% 
  filter(año == "2016-12-01") # aqui era 2014-12-01
...
if (usar_referencia) {
  muns_acteco <- read_csv("../data/referencias/mun_luces_175.csv",
    ...
    left_join(by="CVEENT", 
        pibe_16 %>% select(CVEENT, Estado, pibe)) %>% 
...
write_csv(muns_acteco %>% select(-pibe, -CVEENT), 
  "../data/resultados/acteco/por_municipio_actualizado_2016.csv")
...
ggsave(plot = gg_lineup, 
    "../visualization/figures/scatter_luces_log_actualizado_2016.png", 
...
write_csv(acteco_metro, 
  "../data/resultados/acteco/por_zonas_metro_actualizado_2016.csv")
