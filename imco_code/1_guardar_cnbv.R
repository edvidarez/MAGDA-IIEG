...
leer_multibanca <- function (pestagna_id, periodo, output = FALSE, bancos = NULL) {
    require(zoo)
    require(readxl)
...
pestagnas_df <- read_csv("../data/referencias/pestañas_cnbv.csv", locale = locale(encoding = "latin1")) %>% 
    filter(Alias == "transacciones_atm")

periodos <- seq(empieza, termina, by = "1 month") %>% format("%Y%m")


MB_frame_ <- expand.grid(pestagnas_df$Nombre, periodos, stringsAsFactors=FALSE) %>%
    apply(1, . %>% {
        leer_multibanca(.[1], .[2], output=TRUE, bancos = c("BBVA Bancomer", "Banamex") )
    }) 
